...
if ( ! class_exists( 'YITH_Sequential_Order_Number_Admin' ) ) {
	...
	class YITH_Sequential_Order_Number_Admin {
		...
		public function add_extra_classes( $classes, $field ) {
			...
			if ( isset( $_GET['page'] ) && $this->_panel_page == $_GET['page'] ) {
				$rows_ids = array(
					'ywson_enable_base_module',
					'ywson_check_custom_free_order',
					'ywson_check_custom_quote_order'
				);


				$field_id = $field['id'];

				if ( in_array( $field_id, $rows_ids ) ) {
					$classes[] = 'closed';
				} else {
					$classes[] = 'closed_item';
				}

				if ( 'ywson_check_custom_quote_order' == $field_id ) {

					if ( ! ywson_is_raq_active() ) {
