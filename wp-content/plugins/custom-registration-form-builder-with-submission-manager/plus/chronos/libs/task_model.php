...
class RM_Chronos_Task_Model {
    ...
    public function load_from_db($task_id) {
        ...
        $db_row = $wpdb->get_row("SELECT * from `$table_name` where `task_id` = $task_id");               
        if(!$db_row)
            return false;
    ...
    public function insert_into_db() {
        ...
        $table_name = RM_Chronos::get_table_name_for('TASKS');
        $max_order = $wpdb->get_var("SELECT MAX(`task_order`) FROM {$table_name} WHERE `form_id` = {$data['form_id']}");
        if($max_order!== NULL)
